...
bool Comparison::isJoin(){
    if( (operand1 == Left || operand1 == Right) && (operand2 == Left || operand2 == Right) && (op != LessThan && op != GreaterThan))
        return true;
    else
        return false;
}
...
OrderMaker :: OrderMaker(Schema *schema, Attribute* atts, int n) {
	numAtts = 0;


	for (int i = 0; i < n; i++) {
		if (atts[i].myType == Int) {
			whichAtts[numAtts] = schema->Find(atts[i].name);
			whichTypes[numAtts] = Int;
			numAtts++;
		}
	}
